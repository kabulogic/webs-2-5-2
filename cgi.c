 ...
 */
	...
	*(argp+n) = NULL;
/*
 *	如果是上传文件的方式 那么直接将文件数据入到指定的目录中 并将文件目录写入环境变量中
 */
	printf("websCgiHandler::wp->flag&WEBS_MULTIPART_FORM-->>%d\n",wp->flags&WEBS_MULTIPART_FORM);
	printf("websCgiHandler::lenPostData-->%d\n", wp->lenPostData);
	printf("websCgiHandler::postdata-->%s\n", wp->postData);
	if( wp->flags & WEBS_MULTIPART_FORM) {
    	...
    	int          numLeft;
    	int          numWrite;
		if ((fp = fopen((bn == NULL ? "/home/zeed/upldForm.bin" : bn), "w+b")) == NULL) {
        	printf("websCgiHandler::fopen failed\n");
    	} else {
        	locWrite = 0;
        	...
        	while (numLeft > 0) {
            	...
            	if (numWrite < numLeft) {
                	printf("websCgiHandler::File write failed.ferror=%d locWrite=%d numLeft=%d numWrite=%d Size=%d bytes\n" ,
						ferror(fp), locWrite, numLeft, numWrite, wp->lenPostData);
            		break;
            	}
            	locWrite += numWrite;
            	numLeft -= numWrite;
        	...
        	if (numLeft == 0) {
            	if (fclose(fp) != 0) {
                	printf("websCgiHandler::File close failed.errno=%d locWrite=%d numLeft=%d numWrite=%d Size=%d bytes\n",
						errno, locWrite, numLeft, numWrite, wp->lenPostData);
            	} else {
                	printf("websCgiHandler::File Size Written = %d bytes\n", wp->lenPostData);
            	}
        	} else {
            	printf("websCgiHandler::numLeft=%d locWrite=%d Size=%d bytes\n", numLeft, locWrite, wp->lenPostData);
        	}
    	}
	}
/*
 ...
 */
 	printf("websCgiHandler::stdIn-->%s\n", stdIn);
	printf("websCgiHandler::stdOut-->%s\n", stdOut);
