...
#define UM_PORTA		T("portAccess")
#define UM_USERM		T("userManager")
#define UM_DIVICES		T("diviceSetting")
#define UM_SECURITYM	T("securityManager")
#define UM_SYSTEMM		T("systemMaintenance")
/*
 *	XOR encryption mask
 ...
 */
#define NUMBER_OF_GROUP_COLUMNS	6

char_t	*groupColumnNames[NUMBER_OF_GROUP_COLUMNS] = {
			UM_NAME, UM_PORTA, UM_USERM, UM_DIVICES, UM_SECURITYM, UM_SYSTEMM
};
...
//c  	dbRegisterDBSchema(&accessTable);
	}
...
#endif
/*my add user group name*/
int	umAddGroup(char_t *group, bool_t bportA, bool_t buserM, bool_t bdiviceS, 
			   bool_t bsecurityM, bool_t bsystemM)
{
	int row;
	a_assert(group && *group);
	trace(3, T("UM: Adding group <%s>\n"), group);
 ...
 *	Write the remaining fields
 */
	dbWriteInt(didUM, UM_GROUP_TABLENAME, UM_PORTA, row, bportA);
	dbWriteInt(didUM, UM_GROUP_TABLENAME, UM_USERM, row, buserM);
	dbWriteInt(didUM, UM_GROUP_TABLENAME, UM_DIVICES, row, bdiviceS);
	dbWriteInt(didUM, UM_GROUP_TABLENAME, UM_SECURITYM, row, bsecurityM);
	dbWriteInt(didUM, UM_GROUP_TABLENAME, UM_SYSTEMM, row, bsystemM);
...
/*============================================================================*/
/*
 *	Returns row if success
 */

int umSearchStr(char_t* tablename, char_t* columnname, char_t* userGroupName)
{
	return dbSearchStr(didUM, tablename, columnname, userGroupName, 0);
}
...
int umReadInt(char_t* tablename, char_t* um, int row)
{
	int value;
	dbReadInt(didUM, tablename, um, row, &value);
	return value;
}
/*
 *  Write value to the table (int) 
 */

int umWriteInt(char_t* tablename, char_t* um, int row, int iData)
{
	return dbWriteInt(didUM, tablename, um, row, iData);
}
/*
 *	Write value to the table (string)
 */

int umWriteStr(char* tablename, char_t* um, int row, char_t* str)
{
	return dbWriteStr(didUM, tablename, um, row, str);
}

int umDeleteRow(char* tablename, int row)
{
	return dbDeleteRow(didUM, tablename, row);
}
int umReadStr(char_t* tablename, char_t* um, int row, char_t** returnValue)
{
	return dbReadStr(didUM, tablename, um, row, returnValue);
}
/*
 *  设置用户名
 */
int	umSetUserName(char_t *user, char_t *newuser)
{
	int row;
...
*/
	...
	if (row >= 0) {
		return dbWriteStr(didUM, UM_USER_TABLENAME, UM_NAME, row, newuser);
	} else {
		return UM_ERR_NOT_FOUND;
	}
}
