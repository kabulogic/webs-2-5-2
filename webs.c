 ...
 */
			printf("websReadEvent::NO_CGI_BIN\n");
			printf("websReadEvent::wp->cgiStdin-->>%s\n", wp->cgiStdin);
...
/*				wp->query = bstrdup(B_L, text);*/
				printf("websReadEvent::memdup-->postData\n");
                wp->postData = bmemdup(B_L, text, nbytes);
                wp->lenPostData = nbytes;
				printf("websReadEvent::lenPostData-->>%d\n", wp->lenPostData);
			}
/*
...
void websSetEnv(webs_t wp)
{
	...
	if (wp->flags & WEBS_MULTIPART_FORM) {
		...
		printf("websSetEnv::uploadFileContent-->>%s\n", uploadedFileContent);
